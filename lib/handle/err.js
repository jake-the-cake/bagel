...
function handleError(status) {
  ...
  return createError(status === "NaN" ? DEFAULT_STATUS : status);
}

function errorMiddleware405(req, res, next) {
  req.method !== "GET" &&
    !req.path.startsWith(useEnv("API_PATH")) &&
    res.out.flag(useError("getOnly405"));
  next();
}

function errorMiddleware404(_, res) {
